...
{
	CBLDatabase* db = base->core.db;
	FLString flCollectionName = fl_from_c(collectionName);
	FLString flScopeName = fl_from_c(scopeName);
	CBLError err = (CBLError){0};
	bool bResult = false;
	
	CBLCollection* txc = CBLDatabase_CreateCollection(db, flCollectionName, flScopeName, &err);
	if (NULL != txc)
	{
		CBLCollection_Release(txc);
		bResult = true;
	}
	return bResult;
}
...
bool cblu_open_collection(CBLU_Db* base, const char* scopeName, const char* collName, CBLU_Db** out_handle) {
	...
	*out_handle = NULL;
	...
	coll = CBLDatabase_Collection(base->core.db, fl_from_c(collName), fl_from_c(scopeName), &err);
